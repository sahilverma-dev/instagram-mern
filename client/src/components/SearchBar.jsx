...
const SearchBar = () => {
  ...
  const fetchUsers = async (term) => {
    ...
    const { data } = await axios.get(
      `${API_BASE}/api/v1/user/all?query=${term}`
    );
    setUsers(data.users);
    setLoading(false);
  };
  ...
  return (
    <div onBlur={handleClickAway}>
      <div className="z-10 hidden sm:flex items-center gap-3 pl-4 ml-36 w-64 py-2 bg-[#efefef] rounded-lg relative">
